...
class TestOllamaHealthCheck:
    ...
    def test_validate_ollama_setup_success(self):
        ...
        with patch('smart_agent.ollama_health.check_ollama_service', return_value=True), \
             patch('smart_agent.ollama_health.check_model_availability', return_value=True):
    ...
    def test_validate_ollama_setup_service_not_running(self):
        ...
        with patch('smart_agent.ollama_health.check_ollama_service', return_value=False):
    ...
    def test_validate_ollama_setup_model_not_available(self):
        ...
        with patch('smart_agent.ollama_health.check_ollama_service', return_value=True), \
             patch('smart_agent.ollama_health.check_model_availability', return_value=False), \
             patch('smart_agent.ollama_health.get_available_models', return_value=["codellama:7b"]):
            
            with pytest.raises(OllamaHealthError) as exc_info:
                validate_ollama_setup()
            
            assert f"Required model '{DEFAULT_MODEL}' is not available" in str(exc_info.value)
    ...
    def test_validate_ollama_setup_custom_model(self):
        ...
        with patch('smart_agent.ollama_health.check_ollama_service', return_value=True), \
             patch('smart_agent.ollama_health.check_model_availability', return_value=True) as mock_check:
